...
export const CASES: Case[] = [
  ...
  },
  {
    ...
    instructorNote: `LLMs are best understood as a <strong>hybrid or ambiguous case</strong> depending on how we consider the relevant performance dimension or customer segment. Initial versions of ChatGPT were prone to hallunication and therefore unreliable for enterprise use or anything that requires accuracy. In that sense, it was inferior to search for most mainstream searchers. More importantly, Google's core customer--advertisers--were not very excited about LLMs because they drastically reduced traffic to their websites. This is most likely the reason that Google, despite having far advanced model capabilities than the startup OpenAI was far behind in releasing LLMs to the public. However, what makes this case somewhat interesting is how quickly the capabilities have advanced and surpassed the performance of search engines. This has allowed Google to aggressively integrate LLMs into search without having to worry about losing their competitive positioning. The irony is that if LLMs advanced a slower pace, it would have been far more dangerous to Google than the current situation. `,
    questions: BASE_QUESTIONS({
      ...
      foothold: 'Are LLMs initially serving overlooked or non-consuming users who previously couldn\'t afford or were not satisfied with Google search, or are they serving mainstream users of Google search?',
